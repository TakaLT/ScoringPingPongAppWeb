...
<html>
    ...
    <body>
        ...
        if ( ( $_GET['action']=='modif' ) && ( $_GET['id_configuration']!='' )  ) 
          {
 
            //alimentation des champs 
            $req="SELECT  * FROM parametre join configuration on parametre.id_parametre=configuration.id_parametre WHERE id_configuration='".$_GET['id_configuration']."'";
            $req=$bdd-> query ($req);
            $parametre = $req ->fetch();
        ...
        if (isset($_POST['modif_parametre']))
        {
            ...
            if ($_GET['id_configuration']!=1 )
                ...
                //Mise a jour 
                $req='UPDATE configuration SET nom_configuration= :nom   WHERE id_configuration = :id_configuration ';
                $req_modif= $bdd->prepare($req);
                $req_modif->execute (array('nom'=>$_POST['nom_theme'], 'id_configuration'=>$_GET['id_configuration']));

                //reccuperation id_parametre
                $req="SELECT  * FROM configuration join parametre on configuration.id_parametre=parametre.id_parametre WHERE configuration.id_configuration='".$_GET['id_configuration']."'";
                $req=$bdd-> query ($req);
                $parametre = $req ->fetch();

                $id_para=$parametre['id_parametre'];
                ...
                $req_modif= $bdd->prepare($req);
                $req_modif->execute (array('couleur_f'=>$couleurf, 'couleur_t'=>$couleurt, 'taille'=>$_POST['taille'], 'gras'=>$_POST['gras'], 'style'=>$style, 'id_parametre'=>$id_para));
        ...
        <header class="page-header">
            <h3><?=__Titre_modif_configuration ?></h3>
        </header>
        <div class="row">
            ...
            <form method='POST' action='#'>
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="couleur_text" id="couleur_text_mod" class="colorpicker form-control" value="<?php echo $parametre['couleur_text'] ?>"/>
                </div>
                <div class="form-group">
            ...
            <div class="form-group">    
                 <h3> <?=__Table_t ?> </h3>
                <table id="TabTournoi" class="tablesorter tablesorter">
                    <thead>
                    ...
                    <tbody>
                     <?php 
                                ...
                                while ($para=$req_para->fetch())
                                    {
                                ?>
                        <tr> <td> <?=$para['id_parametre']?> </td> <td> <?=$para['nom_configuration']?> </td><td> <?=$para['couleur_fond']?> </td> <td> <?=$para['couleur_text']?> </td> <td> <?=$para['taille_text']?> </td><td> <?=$para['gras']?> </td><td> <?=$para['style_text']?> </td> <td> <a href="modification_configuration.php?action=modif&id_configuration=<?=$para['id_parametre']?>"> modification </a>/ <a href="supprime_configuration.php?action=supp&id_configuration=<?=$para['id_parametre']?>">supprimer</a></td> </tr>
                                <?php
                                }
