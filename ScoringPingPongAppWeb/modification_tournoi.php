...
<html>
    ...
    <body>
        ...
        if ( ( $_GET['action']=='modif' ) && ( $_GET['id_tournoi']!='' ) ) 
          {
            //alimentation des champs
            $req="SELECT * FROM tournoi WHERE id_tournoi='".$_GET['id_tournoi']."'";
            $req=$bdd-> query ($req);
            $tournoi= $req ->fetch();
               
            
            if (isset($_POST['modif_tournoi'])) 
                {
             ...
             if ($nbreligne < 1)                 
                ...
                $req='UPDATE tournoi SET nom_tournoi = :nom,  date_debut_tournoi = :debut, date_fin_tournoi = :fin, nbre_manche_tournoi = :manche, type_tournoi = :type , nbre_table_tournoi = :table, nbe_tour_tournoi = :tour, id_configuration = :id_conf WHERE id_tournoi = :id_tournoi ';
                $req_tournoi= $bdd->prepare($req);
                $req_tournoi->execute (array('nom'=>$nom_tournoi, 'debut'=>$date_debut, 'fin'=>$date_fin, 'manche'=>$nbre_manche,'type'=>$type_tournoi ,'table'=>$nbre_table,'tour'=>$nbre_tour, 'id_conf'=>$_POST['theme'], 'id_tournoi'=>$_GET['id_tournoi']));
        ...
        <header class="page-header">
            <h2><?=__Titre_modif_tournoi ?> </h2>
        </header> 
        <div class="row">
            <div class="col-lg-2 col-lg-offset-5"> 
            <form method="POST" action="#" >
                <div class="form-group">
                    <label for="nom_tour_mod"><?=__Nom_tournoi ?> :</label><br/>
                    <input type="text" name="nom_tournoi" id="nom_tournoi_mod" class="form-control" value="<?php echo $tournoi['nom_tournoi'] ?>" required="required"/>
                </div>
                <div class="form-group">
                    <label for="datepicker1_mod"><?=__Date_d ?> :</label><br/>
                    <input type="text" name="date_debut" id="datepicker1_mod" class="datepicker form-control" value="<?php echo $tournoi['date_debut_tournoi'] ?>" required="required"/>
                </div>
                <div class="form-group">
                    <label for="datepicker2_mod"><?=__Date_f ?> :</label><br/>
                    <input type="text" name="date_fin" id="datepicker2_mod" class="datepicker form-control" value="<?php echo $tournoi['date_fin_tournoi'] ?>" required="required"/>
                </div>
                <div class="form-group">
                    <label for="type_tournoi_mod"><?=__Type_t ?> :</label><br/>
                    <input type="text" name="type_tournoi" id="type_tournoi_mod" class="form-control" value="<?php echo $tournoi['type_tournoi'] ?>" placeholder="<?=__Fac ?>"/>
                </div>
                <div class="form-group">
                    <label for="nbr_manche_mod"><?=__Nbre_m ?> :</label><br/>
                        <input type="text" name="nbr_manche" id="nbr_manche_mod" class="form-control" value="<?php echo $tournoi['nbre_manche_tournoi'] ?>" required="required"/>
                </div>
                <div class="form-group">
                    <label for="nbr_tour_mod"><?=__Nbre_to ?> :</label><br/>
                    <input type="text" name="nbr_tour" id="nbr_tour_mod" class="form-control" value="<?php echo $tournoi['nbe_tour_tournoi'] ?>" required="required"/>
                </div>
                <div class="form-group">
                    <label for="nbr_table_mod"><?=__Nbre_ta ?> :</label><br/>
                    <input type="text" name="nbr_table" id="nbr_table_mod" class="form-control" value="<?php echo $tournoi['nbre_table_tournoi'] ?>" required="required"/>
                </div>
                <div class="form-group">
                    <label for="theme_mod"><?=__Choix_t ?> :</label><br/>
                    <select name="theme" id="theme_mod" class="form-control">
                        <?php
                ...
                <div class="form-group">
                    <input type="reset" name="reset" class="btn btn-default" value="<?=__Reset ?> "/> <input type="submit" name="modif_tournoi" class="btn btn-default" value="<?=__Modifier ?> "/><br/>
                </div>               
            </form>
                <a href="menu.php" ><?=__Retour ?></a>
                <a href="tournoi.php" ><?=__Retour_tournoi ?></a>
            </div>    
            </div>
            <div class="form-group">
                <h3> <?=__Table_t ?> </h3>
        ...
        <script type="text/javascript">
            $(document).ready(function()  { 
                ...
                $( "#datepicker1_mod" ).datepicker({dateFormat: 'yy-mm-dd', firstDay:1,
                defaultDate: "+1w",
                changeMonth: true,
                numberOfMonths: 1,
                onClose: function( selectedDate ) {
                $( "#datepicker2_mod" ).datepicker( "option", "minDate", selectedDate );
                }
                });
                $( "#datepicker2_mod" ).datepicker({dateFormat: 'yy-mm-dd', firstDay:1,
                defaultDate: "+1w",
                changeMonth: true,
                numberOfMonths: 1,
                onClose: function( selectedDate ) {
                $( "#datepicker1_mod" ).datepicker( "option", "maxDate", selectedDate );
                }
                });
